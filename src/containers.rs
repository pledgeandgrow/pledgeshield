    ...
    if let Ok(output) = info_output {
        if output.status.success() {
            ...
            if !security_opts.contains("userns") && !security_opts.contains("userns-remap") {
                findings.push(
                    Finding::new("ctr-docker-no-userns", "Docker User Namespace Remapping Disabled", Severity::Medium, Category::Services)
            ...
            if !security_opts.contains("seccomp") {
                findings.push(
                    Finding::new("ctr-docker-no-seccomp", "Docker Seccomp Not Enforced", Severity::Medium, Category::Services)
            ...
            if !security_opts.contains("apparmor") && !security_opts.contains("selinux") {
                findings.push(
                    Finding::new("ctr-docker-no-mac", "Docker No Mandatory Access Control", Severity::Low, Category::Services)
    ...
    if std::path::Path::new(socket_path).exists() {
        ...
        if let Ok(meta) = metadata {
            #[cfg(unix)]
            {
                ...
                if perms & 0o077 != 0 {
                    findings.push(
                        Finding::new("ctr-docker-socket-permissive", "Docker Socket Has Permissive Permissions", Severity::High, Category::Services)
...
fn check_podman_security() -> Vec<Finding> {
    ...
    if let Ok(output) = output {
        if output.status.success() {
            ...
            if rootless == "false" {
                findings.push(
                    Finding::new("ctr-podman-rootful", "Podman Running in Rootful Mode", Severity::Low, Category::Services)
    ...
    if let Ok(output) = output {
        if output.status.success() {
            ...
            if has_false || has_empty {
                findings.push(
                    Finding::new("ctr-k8s-pods-root", "Kubernetes Pods Running as Root", Severity::Medium, Category::Services)
    ...
    if let Ok(output) = limits_output {
        if output.status.success() {
            ...
            if limits.trim().is_empty() {
                findings.push(
                    Finding::new("ctr-k8s-no-limits", "Kubernetes Pods Without Resource Limits", Severity::Low, Category::Services)
