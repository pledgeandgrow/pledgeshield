    ...
    #[cfg(target_os = "linux")]
    {
            ...
            for line in s.lines().skip(1) {
                ...
                if let Some((ip, port)) = peer.rsplit_once(':') {
                    ...
                    if let Ok(p) = port.parse::<u16>() {
                        let suspicious_ports = [4444, 5555, 6666, 7777, 8888, 9999, 1337, 31337, 1234, 4321];
                    ...
                    if ip.starts_with("10.") || ip.starts_with("192.168.") {
                        ...
                        if line.contains("users:") {
                            let proc_name = line.split("users:").nth(1)
                                .and_then(|s| s.split('"').nth(1))
                                .unwrap_or("unknown");
                            // Flag if it's an unusual process making external connections
                            let unusual = ["nc", "ncat", "netcat", "socat", "python", "perl", "ruby", "bash", "sh"];
        ...
        if let Ok(o) = out {
            ...
            for line in s.lines().skip(1) {
                ...
                let local = parts[3];
                if local.starts_with("0.0.0.0:") || local.starts_with("*:") || local.starts_with("[::]:") {
...
pub fn list_connections() -> Vec<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if let Ok(o) = out {
            let s = String::from_utf8_lossy(&o.stdout);
            lines.push(format!("{:<8} {:<22} {:<22} {}", "PROTO", "LOCAL", "PEER", "PROCESS"));
            for line in s.lines().skip(1) {
                let parts: Vec<&str> = line.split_whitespace().collect();
                if parts.len() < 5 { continue; }
                let proto = parts[0];
                ...
                let local = parts.get(4).unwrap_or(&"");
                let peer = parts.get(5).unwrap_or(&"");
                let proc = line.split("users:").nth(1)
                    .and_then(|s| s.split('"').nth(1))
                    .unwrap_or("-");
