    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if let Some(cache) = cache_path {
            if let Ok(out) = Command::new("du")
                .args(["-sh", cache.to_str().unwrap_or(".")])
                .output()
            {
            ...
            if let Some(size) = s.split_whitespace().next() {
                if size.contains("G") {
                    findings.push(
                        ...
                        .description(
                            "Log directory is using significant space. Consider log rotation.",
                        ),
                    );
                }
            }
...
pub fn monitor_disk_usage(interval: u64, max_runtime: u64) {
    ...
    println!(
        "  [baseline] Free space: {:.1} GB",
        prev_free as f64 / 1073741824.0
    );
    ...
    loop {
        ...
        if delta < -1073741824 {
            // > 1GB decrease
            println!(
                "  {} [HIGH] Disk space dropped by {:.1} GB — possible mass encryption!",
                now,
                (-delta as f64) / 1073741824.0
            );
        } else if delta < -104857600 {
            // > 100MB decrease
            println!(
                "  {} [medium] Disk space decreased by {:.0} MB",
                now,
...
fn get_disk_free() -> u64 {
    #[cfg(target_os = "linux")]
    {
        let out = Command::new("sh")
            .args(["-c", "df -B1 --output=avail / | tail -1"])
            .output();
        if let Ok(o) = out {
