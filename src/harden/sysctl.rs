...
const SECURITY_SYSCALLS: &[(&str, &str, &str)] = &[
    ...
    (
        ...
        "0",
        "Don't accept ICMP redirects (default)",
    ),
    (
    ...
    (
        ...
        "0",
        "Reject source-routed packets (default)",
    ),
    (
    ...
    (
        ...
        "0",
        "Disable user namespaces (reduces attack surface)",
    ),
    (
    ...
    (
        ...
        "2",
        "Protect against regular file attacks",
    ),
                ...
                if current != *secure_val && !current.is_empty() {
                    findings.push(
                        ...
                        .recommendation(&format!(
                            "Run: pledgeshield harden sysctl --harden  (or: sudo sysctl -w {}={})",
                            param, secure_val
                        ))
                        .fixable(true),
                    );
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        for (param, secure_val, _) in SECURITY_SYSCALLS {
            ...
            let out = Command::new("sysctl")
                .args(["-w", &format!("{}={}", param, secure_val)])
                .output();
        ...
        if !dry_run && fixed > 0 {
            ...
            let conf: String = SECURITY_SYSCALLS
                .iter()
            ...
            results.push(HardenResult {
                ...
                success: true,
                message: format!(
                    "Set {} kernel parameters (persisted to /etc/sysctl.d/99-pledgeshield.conf)",
                    fixed
                ),
                findings: vec![],
            });
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if std::path::Path::new(path).exists() {
            ...
            HardenResult {
                ...
                success: true,
                message: "Removed PledgeShield sysctl config (reboot to fully restore defaults)."
                    .to_string(),
                findings: vec![],
            }
        } else {
