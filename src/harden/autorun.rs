...
pub fn audit_autorun() -> Vec<Finding> {
    ...
    #[cfg(windows)]
    {
        ...
        let out = Command::new("reg")
            .args(["query", "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", "/v", "NoDriveTypeAutoRun"])
            .output();
        if let Ok(o) = out {
            ...
            if !s.contains("0xff") && !s.contains("0xFF") {
                ...
                .description("AutoRun allows malware on USB/CD to execute automatically when inserted.")
        ...
        let out = Command::new("powershell")
            .args(["-Command", "Get-CimInstance -ClassName Win32_AutochkSetting | Select-Object SettingID"])
            .output();
        // Also check via registry
        let out = Command::new("reg")
            .args(["query", "HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\AutoplayHandlers", "/v", "DisableAutoplay"])
            .output();
        if let Ok(o) = out {
            let s = String::from_utf8_lossy(&o.stdout);
            if !s.contains("0x1") {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if let Ok(o) = out {
            ...
            if s.contains("false") {
                ...
                .recommendation("Run: gsettings set org.gnome.desktop.media-handling autorun-never true")
                .fixable(true));
            }
        }
    }
...
pub fn disable_autorun(dry_run: bool) -> HardenResult {
    ...
    #[cfg(windows)]
    {
        ...
        let out1 = Command::new("reg")
            .args(["add", "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer",
                   "/v", "NoDriveTypeAutoRun", "/t", "REG_DWORD", "/d", "255", "/f"])
            .output();

        // Disable AutoPlay
        let out2 = Command::new("reg")
            .args(["add", "HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\AutoplayHandlers",
        ...
        HardenResult {
            action: "autorun-disable".to_string(),
            success,
            message: if success { "AutoRun and AutoPlay disabled for all drives.".to_string() }
                     else { "Failed to disable AutoRun (need admin?)".to_string() },
            findings: vec![],
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        let _ = Command::new("gsettings")
            .args(["set", "org.gnome.desktop.media-handling", "automount", "false"]).output();
        let _ = Command::new("gsettings")
            .args(["set", "org.gnome.desktop.media-handling", "autorun-never", "true"]).output();
        HardenResult {
            action: "autorun-disable".to_string(),
            success: true,
