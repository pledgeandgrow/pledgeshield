            ...
            for entry in entries.flatten() {
                ...
                if !exe_basename.is_empty() {
                    ...
                    if comm_truncated != exe_truncated {
                        ...
                        let known_safe = [
                            "python", "bash", "sh", "dash", "node", "ruby", "perl", "java",
                            "sleep", "watch", "timeout", "xargs", "find", "grep", "awk", "sed",
                            "sort", "head", "tail", "cat", "ls", "ps", "ss", "systemd", "dbus",
                            "cron",
                        ];

                        if !known_safe.contains(&comm.as_str())
                            && !known_safe.contains(&exe_basename)
                        {
