...
pub fn audit_telemetry() -> Vec<Finding> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if std::path::Path::new("/usr/sbin/popularity-contest").exists() {
            let out = Command::new("deb-systemd-helper").args(["--user", "is-enabled", "popularity-contest.service"]).output();
            if let Ok(o) = out {
                let s = String::from_utf8_lossy(&o.stdout);
        ...
        let out = Command::new("systemctl").args(["is-enabled", "whoopsie"]).output();
        if let Ok(o) = out {
            let s = String::from_utf8_lossy(&o.stdout);
            if s.contains("enabled") {
                findings.push(Finding::new(
                    ...
                    .description("whoopsie sends crash reports to Ubuntu, which may contain sensitive data.")
                    .fixable(true));
            }
        }
    }
    ...
    #[cfg(windows)]
    {
        ...
        let out = Command::new("reg")
            .args(["query", "HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection", "/v", "AllowTelemetry"])
            .output();
        if let Ok(o) = out {
            let s = String::from_utf8_lossy(&o.stdout);
        ...
        // Cortana
        let out = Command::new("reg")
            .args(["query", "HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\Windows Search", "/v", "AllowCortana"])
            .output();
        if let Ok(o) = out {
            let s = String::from_utf8_lossy(&o.stdout);
            if !s.contains("0x0") {
                ...
                for entry in entries.flatten() {
                    ...
                    if prefs.exists() {
                        if let Ok(content) = std::fs::read_to_string(&prefs) {
                            if !content.contains("toolkit.telemetry.enabled") || content.contains("toolkit.telemetry.enabled, true") {
                                findings.push(Finding::new(
                                    "telemetry-firefox",
...
pub fn clean_telemetry(dry_run: bool) -> Vec<HardenResult> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        results.push(HardenResult {
            ...
            success: true,
            message: if dry_run { "[dry-run] Would disable Ubuntu telemetry".to_string() }
                     else { "Ubuntu telemetry disabled".to_string() },
            findings: vec![],
        });
        ...
        if std::path::Path::new("/etc/default/motd-news").exists() {
            ...
            results.push(HardenResult {
                ...
                success: true,
                message: if dry_run { "[dry-run] Would disable MOTD news".to_string() }
                         else { "MOTD news disabled".to_string() },
                findings: vec![],
            });
        }
        ...
        // whoopsie
        if !dry_run {
            let _ = Command::new("systemctl").args(["disable", "--now", "whoopsie"]).output();
        }
        results.push(HardenResult {
            action: "telemetry-whoopsie".to_string(),
            success: true,
            message: if dry_run { "[dry-run] Would disable whoopsie".to_string() }
                     else { "whoopsie error reporting disabled".to_string() },
            findings: vec![],
        });

        // popularity-contest
        if !dry_run {
            let _ = Command::new("systemctl").args(["disable", "--now", "popularity-contest"]).output();
        }
        results.push(HardenResult {
            action: "telemetry-popcon".to_string(),
            success: true,
            message: if dry_run { "[dry-run] Would disable popularity-contest".to_string() }
                     else { "popularity-contest disabled".to_string() },
            findings: vec![],
        });
    }
    ...
    {
        if !dry_run {
            let _ = Command::new("reg").args(["add",
                "HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection",
                "/v", "AllowTelemetry", "/t", "REG_DWORD", "/d", "0", "/f"]).output();
            let _ = Command::new("reg").args(["add",
                "HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\Windows Search",
                "/v", "AllowCortana", "/t", "REG_DWORD", "/d", "0", "/f"]).output();
        }
        results.push(HardenResult {
            action: "telemetry-windows".to_string(),
            success: true,
            message: if dry_run { "[dry-run] Would set Windows telemetry to Security level + disable Cortana".to_string() }
                     else { "Windows telemetry set to Security level, Cortana disabled".to_string() },
            findings: vec![],
        });
    }
