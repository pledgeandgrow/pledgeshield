...
pub fn audit_process_tree() -> Vec<Finding> {
    ...
    let suspicious_pairs: &[(&str, &str, Severity, &str)] = &[
        (
            ...
            "bash",
            Severity::High,
            "Browser spawning shell — possible exploit",
        ),
        (
            "chrome",
            "sh",
        ...
        (
            ...
            "powershell",
            Severity::High,
            "Browser spawning PowerShell — possible exploit",
        ),
        (
            "firefox",
            "bash",
        ...
        (
            ...
            "cmd",
            Severity::High,
            "Browser spawning cmd — possible exploit",
        ),
        (
            "office",
            "powershell",
            Severity::Critical,
            "Office app spawning PowerShell — likely macro malware",
        ),
        (
            "soffice",
            "bash",
            Severity::Critical,
            "LibreOffice spawning shell — likely macro malware",
        ),
        (
            "soffice",
            "powershell",
            Severity::Critical,
            "LibreOffice spawning PowerShell — likely macro malware",
        ),
        (
            "winword",
            "cmd",
            Severity::Critical,
            "Word spawning cmd — likely macro malware",
        ),
        (
            "winword",
            "powershell",
            Severity::Critical,
            "Word spawning PowerShell — likely macro malware",
        ),
        (
            "excel",
            "cmd",
            Severity::Critical,
            "Excel spawning cmd — likely macro malware",
        ),
        (
            "outlook",
            "powershell",
            Severity::Critical,
            "Outlook spawning PowerShell — likely email malware",
        ),
        (
            "nginx",
            "bash",
            Severity::High,
            "Web server spawning shell — possible web shell",
        ),
        (
            "apache",
            "bash",
        ...
        (
            ...
            "bash",
            Severity::High,
            "FTP server spawning shell — possible backdoor",
        ),
    ...
    for (_pid, proc_info) in &tree {
        if proc_info.ppid == 0
            && proc_info.name != "kernel"
            && proc_info.name != "systemd"
            && proc_info.name != "launchd"
            && proc_info.name != "idle"
        {
