...
pub fn audit_bootlog() -> Vec<Finding> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let out = Command::new("dmesg").args(["--level=err,crit,emerg"]).output();
        if let Ok(o) = out {
            let s = String::from_utf8_lossy(&o.stdout);
            ...
            if error_count > 20 {
                ...
                .description("Many boot errors may indicate hardware failure, driver issues, or attack."));
            }
        }
    }
